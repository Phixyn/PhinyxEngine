...
PhinyxEngine::Game::Game(
	...
	const bool SHOW_DEBUG_PANE) :
		m_view(sf::Vector2f(WIDTH / 2.0f, HEIGHT / 2.0f),
			sf::Vector2f((float)WIDTH, (float)HEIGHT)),
		m_width(WIDTH),
		m_height(HEIGHT),
		m_title(TITLE),
		m_showDebugPane(SHOW_DEBUG_PANE)
{
...
void PhinyxEngine::Game::clear()
{
	m_window->clear();
}

void PhinyxEngine::Game::render()
{
	// Reset the view
...
void PhinyxEngine::Game::draw(const sf::Drawable &drawable)
{
	m_window->draw(drawable);
}

void PhinyxEngine::Game::drawRect(sf::RectangleShape rect)
{
	m_window->draw(rect);
}

void PhinyxEngine::Game::handleEvents()
{
	sf::Event sfEvent;
...
bool PhinyxEngine::Game::isOpen()
{
	return m_window->isOpen();
}

bool PhinyxEngine::Game::hasFocus()
{
	return m_hasFocus;
}
