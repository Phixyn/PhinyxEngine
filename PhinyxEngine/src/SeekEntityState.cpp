...
PhinyxEngine::SeekEntityState::SeekEntityState(LiveEntity* source, PlayerEntity& target) :
	BaseEntityState("Seeking"), m_sourcePtr(source), m_playerPtr(&target)
{
	enter();
}

void PhinyxEngine::SeekEntityState::enter()
{
	m_sourcePtr->setSpriteColor(sf::Color::Green);
}

void PhinyxEngine::SeekEntityState::update()
{
...
void PhinyxEngine::SeekEntityState::exit()
{
	m_sourcePtr->setVelocity(sf::Vector2f(0.0f, 0.0f));
}

PhinyxEngine::BaseEntityState* PhinyxEngine::SeekEntityState::getNewState()
{
	return nullptr;
}
