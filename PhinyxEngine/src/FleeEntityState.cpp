...
PhinyxEngine::FleeEntityState::FleeEntityState(LiveEntity* source, PlayerEntity& target) :
	BaseEntityState("Fleeing"), m_sourcePtr(source), m_playerPtr(&target)
{
	enter();
}

void PhinyxEngine::FleeEntityState::enter()
{
	m_sourcePtr->setSpriteColor(sf::Color::Red);
}

void PhinyxEngine::FleeEntityState::update()
{
...
void PhinyxEngine::FleeEntityState::exit()
{
	m_sourcePtr->setVelocity(sf::Vector2f(0.0f, 0.0f));
}

PhinyxEngine::BaseEntityState* PhinyxEngine::FleeEntityState::getNewState()
{
	return nullptr;
}
